...
This patch lets trusted Discord admins chat with HORIZON BOT in DM without hardcoding every admin user ID.
...
HORIZON BOT will allow DM chat when the user is one of these:
...
If `SERVER_ADMIN_GUILD_IDS` is set, only those guilds are checked. If it is empty, HORIZON BOT checks every guild it is in.
...
For a trusted friend who only needs to talk with HORIZON BOT:
...
Then give the friend an Admin role in the HORIZON Discord server, or give them Discord Administrator permission.
